...
%% Timed response experiment for EEG study
   script = 'Experiment2_Condition1_AudioCuedRT.txt'; % eg.
%     subjectFolder = '000\B3'; %'001/B8';
%    subjectFolder = '000\B7'; %'001/B8';
% 
%      script = 'Experiment2_Condition2_TimedResponse.txt';
%     subjectFolder = '000\B4'; %'001/B8'; 
     subjectFolder = 'alex\B6';
...
%                 output = [output; x y];
               ...
               if time >= thisTrial.switchTime/1000 && targetOn == 0,
                   if cond == 1
                       io64(ioObj,address,28);
                   elseif cond == 2,
                       io64(ioObj,address,26);
                   end 
                   pause(0.000001);
                   targetOn = 1;
               end
                ...
                if cond == 1,
                   if time > 0.5 && time < 0.5 + timeWindow 
                       io64(ioObj,address,131);   %output command - change this to use thisTrial.trialCode           
                   elseif time > 1.0 && time < 1.0 + timeWindow 
                       io64(ioObj,address,133);   %output command - change this to use thisTrial.trialCode
                   elseif time > 1.5 && time < 1.5 + timeWindow
                       io64(ioObj,address,135);   %output command - change this to use thisTrial.trialCode
                   elseif time > 2 && time < 2 + timeWindow,
                       io64(ioObj,address,137); %buzzer sounds
                   else
                       io64(ioObj,address,0); %buzzer silent
                   end
                elseif cond == 2,
                   if time > 0.5 && time < 0.5 + timeWindow 
                       io64(ioObj,address,141);   %output command - change this to use thisTrial.trialCode           
                   elseif time > 1.0 && time < 1.0 + timeWindow 
                       io64(ioObj,address,143);   %output command - change this to use thisTrial.trialCode
                   elseif time > 1.5 && time < 1.5 + timeWindow
                       io64(ioObj,address,145);   %output command - change this to use thisTrial.trialCode
                   elseif time > 2 && time < 2 + timeWindow,
                       io64(ioObj,address,147); %buzzer sounds
                   else
                       io64(ioObj,address,0); %buzzer silent
                   end
                end
